...
from src.table_page_detector import detect_table_page, detect_target_table
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RapidOCR screening + PaddleOCR-VL structure probe.")
...
def _text_lines_from_ocr(ocr_result: dict[str, Any]) -> list[str]:
    ...
    for item in ocr_result.get("items", []):
        ...
        if stats:
            items.append((stats["y_center"], stats["x_center"], text))
        else:
...
def process_pdf_auto_vl(
    ...
    print(f"\n==> Auto-VL processing {pdf_path.name}")
    ...
    try:
        ...
        write_json(reports_dir / f"{pdf_name}_auto_vl_summary.json", summary)
        return summary
    ...
    for ordinal, image_path in enumerate(image_paths, start=1):
        ...
        try:
            ...
            page_result["table_decision"] = detect_table_page(ocr_result)
            page_result["target_table_decision"] = detect_target_table(ocr_result)
            ...
            _record_error(errors, page_errors, "rapid_text_ocr_or_detection", exc)
            page_result["table_decision"] = {
                "is_table_like": True,
                "reason": "ocr_or_detection_failed_fallback_to_vl",
            }
            page_result["target_table_decision"] = {
                "should_run_paddleocr": True,
                "reason": "ocr_or_detection_failed_fallback_to_vl",
                "policy": "fallback_to_vl",
            }

        table_decision = page_result["table_decision"]
        target_decision = page_result["target_table_decision"]
        should_run_vl = table_decision.get("is_table_like") and target_decision.get("should_run_paddleocr")

        if should_run_vl:
            print(f"    VL: run ({target_decision.get('reason')})")
            try:
            ...
            if not table_decision.get("is_table_like"):
                print(f"    VL: skip ({table_decision.get('reason')})")
            else:
                print(f"    VL: skip ({target_decision.get('reason')})")
...
def _build_summary(
    ...
    vl_pages = [
        page["page_no"]
        for page in pages
        if page.get("vl_summary")
    ]
    target_table_pages = [
        page["page_no"]
        for page in pages
        if page.get("target_table_decision", {}).get("should_run_paddleocr")
    ]
    ...
    return {
        ...
        "target_table_pages": target_table_pages,
        "target_table_page_count": len(target_table_pages),
        "vl_pages": vl_pages,
        "vl_page_count": len(vl_pages),
...
def main() -> None:
    ...
    if args.file and not pdf_files:
        ...
        write_json(reports_dir / "all_files_auto_vl_summary.json", {"file_count": 0, "files": []})
        return
    if not pdf_files:
    ...
    write_json(
        reports_dir / "all_files_auto_vl_summary.json",
        {
    ...
    print(f"\nDone. Auto-VL reports written to {reports_dir}")
